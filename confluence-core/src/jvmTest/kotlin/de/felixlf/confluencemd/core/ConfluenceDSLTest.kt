...
class ConfluenceDSLTest {
    ...
    fun `test code block creation`() {
        ...
        assertTrue(
            confluenceContent.contains("<ac:structured-macro ac:name=\"code\">"),
            "Should contain the code macro"
        )
        assertTrue(
            confluenceContent.contains("<ac:parameter ac:name=\"language\">kotlin</ac:parameter>"),
            "Should specify the kotlin language"
        )
        assertTrue(
            confluenceContent.contains("println(\"Hello, Confluence!\")"),
    ...
    fun `test list creation`() {
        ...
        assertTrue(
            confluenceContent.contains("<strong>special formatting</strong>"),
    ...
    fun `test table creation`() {
        ...
        assertTrue(
            confluenceContent.contains("<strong>Cell 2,2</strong>"),
    ...
    fun `test markdown conversion`() {
        ...
        assertTrue(
            confluenceContent.contains("<h1>Title from Markdown</h1>"),
            "Should contain the converted title"
        )
        assertTrue(
            confluenceContent.contains("<strong>bold text</strong>"),
            "Should contain bold text"
        )
        assertTrue(
            confluenceContent.contains("<em>italic text</em>"),
    ...
    fun `test advanced macros`() {
        ...
        assertTrue(
            confluenceContent.contains("<ac:structured-macro ac:name=\"info\">"),
            "Should contain info panel macro"
        )
        assertTrue(
            confluenceContent.contains("<ac:parameter ac:name=\"title\">Important information</ac:parameter>"),
            "Should contain the info panel title"
        )

        assertTrue(
            confluenceContent.contains("<ac:structured-macro ac:name=\"warning\">"),
            "Should contain warning macro"
        )

        assertTrue(
            confluenceContent.contains("<ac:structured-macro ac:name=\"expand\">"),
            "Should contain expand macro"
        )
        assertTrue(
            confluenceContent.contains("<ac:parameter ac:name=\"title\">See more details</ac:parameter>"),
            "Should contain the expand panel title"
        )

        assertTrue(
            confluenceContent.contains("<ac:structured-macro ac:name=\"toc\">"),
            "Should contain table of contents macro"
        )
        assertTrue(
            confluenceContent.contains("<ac:parameter ac:name=\"maxLevel\">3</ac:parameter>"),
            "Should contain the maximum level for the table of contents"
        )

        assertTrue(
            confluenceContent.contains("<ac:structured-macro ac:name=\"status\">"),
            "Should contain status macro"
        )
        assertTrue(
            confluenceContent.contains("<ac:parameter ac:name=\"colour\">Green</ac:parameter>"),
            "Should contain the status color"
        )
        assertTrue(
            confluenceContent.contains("<ac:parameter ac:name=\"subtle\">true</ac:parameter>"),
    ...
    fun `test links and mentions`() {
        ...
        // Debug: print the actual content for inspection
        println("Generated content: $confluenceContent")

        assertTrue(
            confluenceContent.contains("<ac:link><ri:url ri:value=\"https://example.com\"/><ac:plain-text-link-body>"),
            "Should contain an external link"
        )

        assertTrue(
            confluenceContent.contains("<ac:link><ri:page ri:content-title=\"Home Page\" ri:space-key=\"SPACE\"/>"),
            "Should contain a page link"
        )

        assertTrue(
            confluenceContent.contains("<ac:link><ri:attachment ri:filename=\"document.pdf\" ri:content-title=\"Page with attachments\" ri:space-key=\"SPACE\"/>"),
            "Should contain a reference to an attachment"
        )

        assertTrue(
            confluenceContent.contains("<ac:link><ri:user ri:username=\"admin\"/></ac:link>"),
            "Should contain a user mention"
        )

        assertTrue(
            confluenceContent.contains("<ac:emoticon ac:name=\"thumbsup\"/>"),
    ...
    fun `test from markdown file`() {
        ...
        try {
            ...
            assertTrue(
                confluenceContent.contains("<h1>Test from file</h1>"),
                "Should contain the file title"
            )
            assertTrue(
                confluenceContent.contains("<ul>"),
